...
- **Silent publish failures hide fidelity failures (2026-07-26, ADR-0026 M3 hardening notes — recorded, NOT yet fixed).** (a) `publishObs` (engine/natsio/contract.go) swallows obs publish errors — `obsOut` counts successes only. A 5,000-vehicle obs frame (~1.96 MB with the policy-ctx feature) silently never published under the test broker's then-default 1 MB `max_payload`, stalling the driver with zero log lines; found via the M3 applied-lag harness (the test broker now mirrors the production 4 MiB cap). Recommendation: count obs publish failures loudly, like the snapshot/signal paths' first-3-per-run logging. The same pattern sits in the driver's intent path (`publishIntent`/`flushBatch`, engine/natsio/driver/driver.go): one failed `PublishMsg` silently loses up to 20,000 intents for the tick — hold-last heals ≤ 2 ticks, then the fleet coasts. (b) `NewBus`/`NewContract` subscribe without flushing — a latent subscribe/publish race: a hello or request published from another connection can beat the server-side SUB registration and fast-fail `nats: no responders`, the M1-era explanation for the load-sensitive test flakes (`TestHoldLastThenDecay` and friends under load). **(b) FIXED 2026-08-05:** the constructors now `nc.Flush()` before returning (responders are registered server-side before construction completes), and `RunLive` publishes the registry's running status only AFTER bus and contract are constructed AND the static record plane is out (tick-0 keyframe logged, signal table published) — registry visibility now implies the run is addressable and its static plane exists. Riding along: a construction failure (recorder/bus/contract/keyframe) now returns before `reg.Start`, so a failed run leaves NO registry trace where it previously appeared as aborted-with-error — tooling listing aborted runs will not see these; the caller still gets the error. The per-test `nc.Flush()` workarounds are redundant but harmless; note some flush the *client's* own connection, a different race — check which kind each is before cleaning any up. Still open: a `reg.Start` failure itself orphans the just-written recording (no rollback). (a) — loud publish-failure counting — remains open. Affects: NATS Backbone, Vehicle & Controller Interface.
