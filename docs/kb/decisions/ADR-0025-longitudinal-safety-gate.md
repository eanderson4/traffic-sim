...
**Correction (2026-07-26, external review):** an earlier draft of this ADR
claimed the parameter "participates in the ADR-0012 content hash". It does
not, and cannot as built. The ADR-0012 hash is over the *scenario directory
bytes*; `SafetyDecel` arrives as a `cmd/serve` flag and is written onto
`spec.Params` after `Scenario.RunSpec` has already fixed `spec.Hash`
(scenario.go). So two runs with different safety physics — including gate-on
versus gate-off — share one scenario identity.

That matters wherever the hash is treated as identity: the baked prefix's
`scenarioHash`, and any cache keyed on it. It is recorded here rather than
fixed because the honest fix is a design change (move the gate into the
scenario manifest so it is scenario content, not an invocation flag), and
that is an ADR-0012 amendment, not a patch. Until then, **the scenario hash
does not distinguish gated from ungated runs** — do not use it to tell them
apart.
...
## What it does not cover: a leader braking harder than the bound

The bound solves for the follower's speed on the assumption that the leader
can shed speed no faster than `SafetyDecel` — the same `b` on both sides,
which is what makes the Krauss form closed. That assumption is not enforced
anywhere. A controller may legally intend **−9 m/s²** (`intent.go`, the
validated range), while `serve` runs the gate at `b = 6`. A leader braking
at 9 against a follower whose gap was computed for 6 can still be rear-ended.

So the gate is **not** a collision-prevention guarantee, and this ADR should
not be read as claiming one. What it guarantees is narrower and still worth
having: no vehicle closes on its leader faster than a `b`-bounded stop
allows, which is what removes the coasting- and stale-intent-driven
run-through that motivated it — measured as a 146× reduction, not to zero.

Closing it properly means either clamping intent braking to `SafetyDecel`
while the gate is armed, or solving against the leader's actual commanded
deceleration. Both change the accelerations the kernel produces, and
therefore every recorded CRC. That is a deliberate deferral, not an
oversight: it should land with a re-record, not days before one is used.
