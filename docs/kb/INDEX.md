...
- [ ] [ADR-0029](decisions/ADR-0029-warm-start-from-keyframe.md) — PROPOSED: warm-start a run from an ADR-0005 §5 keyframe instead of an empty network, so an A/B batch pays the 20-minute fill once rather than once per arm. The mechanism mostly exists (keyframes already resume bit-exactly); the blocker is that a keyframe binds vehicles to lane INDEX, so loading one against a widened network silently places every vehicle on a valid wrong lane. Proposes format v4 (lane ids, not indices), an `initial_state:` scenario key, and the keyframe hash entering run identity (the M11 seed-in-hash lesson). Flags the conceptual cost: a warm-started A/B measures response-to-change, not equilibrium-under-change
...
*ADR-0029/ADR-0030 PROPOSED 2026-07-28 (warm start from a keyframe; the standard run report and district map — written after the density distribution showed only 4.6% of freeway lane-km at critical in a run whose corridor means had read as "loaded")*
