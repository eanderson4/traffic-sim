...
| Slow Streets — 25 km/h limit on Loop secondary streets | -0.69 | -8.8% | 0.0000 | -2.06 | WORSE |
...
- **Slow Streets — 25 km/h limit on Loop secondary streets** (`calm-secondary`): WORSE, -8.8% on speed_kmh, p=0.0000 — CAUTION: moves 4.8% less vehicle-distance (p=0.0008), so this is not a like-for-like comparison: the network is doing less work, not doing it better
