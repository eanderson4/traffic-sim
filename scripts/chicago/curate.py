...
def main():
    ...
    ap.add_argument("--json", default=None,
                    help="also write the shortlist as JSON, for mkquiz.py")
    ...
    # The same shortlist, machine-readable, so the guest-facing quiz page is
    # generated from this decision rather than from a second transcription of
    # it. A number that appears in two places diverges the first time one is
    # regenerated; mkquiz.py consumes this.
    if args.json:
        first = shortlist[0] if shortlist else None
        payload = {
            "title": t,
            "baseline": base,
            "metric": metric,
            "seeds": len(rep["seeds"]),
            "ticks": rep["ticks"],
            "winner": name_of[id(first)] if (winners and first) else None,
            "options": [{
                "name": name_of[id(v)],
                "label": labels.get(name_of[id(v)], name_of[id(v)]),
                "delta_pct": v.get("delta_pct"),
                "p": v.get("p"),
                "cohen_d": v.get("cohen_d"),
                "verdict": shown_verdict(v),
                "carries_traffic": carries_its_traffic(v),
            } for v in shortlist],
            "all_tested": {n: {"delta_pct": v.get("delta_pct"), "p": v.get("p"),
                               "verdict": v["verdict"]}
                           for n, v in allv.items()},
        }
        with open(args.json, "w") as f:
            json.dump(payload, f, indent=2)
        print(f"[curate] wrote {args.json}", file=sys.stderr)
